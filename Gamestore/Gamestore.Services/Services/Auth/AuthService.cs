...
public class AuthService
{
    ...
    public async Task<AuthResponseDto?> AuthenticateAsync(string email, string password)
    {
        ...
        _logger.LogWarning(
            "❌ AuthService authentication failed for user: {Email}. Status: {StatusCode}, Error: {Error}",
            email,
            response.StatusCode,
            errorContent);
    ...
    public async Task<bool> CreateUserAsync(string email, string firstName, string lastName, string password)
    {
        ...
        var errorContent = await response.Content.ReadAsStringAsync();
        _logger.LogWarning(
            "Failed to create user in AuthService: {Email}. Status: {StatusCode}, Error: {Error}",
            email,
            response.StatusCode,
            errorContent);
