...
public class CommentsController(ICommentService commentService, ILogger<CommentsController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> AddComment(string key, [FromBody] CommentMetadataRequestDto commentRequest)
    {
        try
        {
            _logger.LogInformation(
                "Adding comment for game with key: {Key} by user: {User} (Role: {Role})",
                key,
                User.GetUserEmail(),
                User.GetUserRole());
            ...
            _logger.LogInformation(
                "Successfully added comment for game {Key} by user {User}",
                key,
                User.GetUserEmail());
    ...
    public async Task<IActionResult> DeleteComment(string key, Guid id)
    {
        try
        {
            ...
            _logger.LogInformation(
                "Deleting comment with ID: {Id} for game with key: {Key} by user: {User} (Role: {Role})",
                id,
                key,
                User.GetUserEmail(),
                User.GetUserRole());
            ...
            _logger.LogInformation(
                "Successfully deleted comment {Id} by user {User}",
                id,
                User.GetUserEmail());
    ...
    public async Task<IActionResult> GetBanDurations()
    {
        ...
        {
            _logger.LogInformation(
                "Getting ban durations for moderator: {User} (Role: {Role})",
                User.GetUserEmail(),
                User.GetUserRole());
    ...
    public async Task<IActionResult> BanUser([FromBody] BanCreateRequestDto banRequest)
    {
        ...
        {
            _logger.LogInformation(
                "Banning user: {User} for duration: {Duration} by moderator: {Moderator} (Role: {Role})",
                banRequest?.User,
                banRequest?.Duration,
                User.GetUserEmail(),
                User.GetUserRole());
            ...
            _logger.LogInformation(
                "Successfully banned user {BannedUser} by moderator {Moderator}",
                banRequest.User,
                User.GetUserEmail());
    ...
    public async Task<IActionResult> CheckUserBanStatus(string userName)
    {
        ...
        {
            _logger.LogInformation(
                "Checking ban status for user: {UserName} by moderator: {Moderator}",
                userName,
                User.GetUserEmail());
