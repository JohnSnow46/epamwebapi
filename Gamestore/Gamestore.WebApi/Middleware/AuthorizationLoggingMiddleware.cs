    ...
    private void LogUserInfo(HttpContext context)
    {
        ...
        if (user.Identity?.IsAuthenticated == true)
        {
            ...
            _logger.LogInformation(
                "🔐 AUTH REQUEST: {Endpoint} | User: {Email} | Role: {Role} | Name: {Name}",
                endpoint,
                userEmail,
                userRole,
                userName);
        ...
        {
            _logger.LogInformation(
                "🔓 ANONYMOUS REQUEST: {Endpoint} | IP: {IP}",
                endpoint,
    ...
    private void LogAuthorizationResult(HttpContext context)
    {
        ...
        if (user.Identity?.IsAuthenticated == true)
        {
            ...
            switch (response.StatusCode)
            {
                case 401:
                    _logger.LogWarning(
                        "{Icon} UNAUTHORIZED: {Endpoint} | User: {Email} | Role: {Role} | Status: {Status}",
                        statusIcon,
                        endpoint,
                        userEmail,
                        userRole,
                ...
                case 403:
                    _logger.LogWarning(
                        "{Icon} FORBIDDEN: {Endpoint} | User: {Email} | Role: {Role} | Status: {Status}",
                        statusIcon,
                        endpoint,
                        userEmail,
                        userRole,
                ...
                case >= 400:
                    _logger.LogWarning(
                        "{Icon} ERROR: {Endpoint} | User: {Email} | Role: {Role} | Status: {Status}",
                        statusIcon,
                        endpoint,
                        userEmail,
                        userRole,
                ...
                default:
                    _logger.LogInformation(
                        "{Icon} SUCCESS: {Endpoint} | User: {Email} | Role: {Role} | Status: {Status}",
                        statusIcon,
                        endpoint,
                        userEmail,
                        userRole,
        ...
        else
        {
            switch (response.StatusCode)
            {
                case 401:
                    _logger.LogInformation(
                        "{Icon} ANONYMOUS DENIED: {Endpoint} | Status: {Status} (Authentication required)",
                        statusIcon,
                        endpoint,
                        response.StatusCode);
                    break;
                case 403:
                    _logger.LogWarning(
                        "{Icon} ANONYMOUS FORBIDDEN: {Endpoint} | Status: {Status}",
                        statusIcon,
                        endpoint,
                        response.StatusCode);
                    break;
                default:
                    _logger.LogInformation(
                        "{Icon} ANONYMOUS ACCESS: {Endpoint} | Status: {Status}",
                        statusIcon,
                        endpoint,
                        response.StatusCode);
                    break;
        ...
        {
            _logger.LogWarning(
                "🚨 SECURITY: Attempted access to admin endpoint {Endpoint} by {User}",
                endpoint,
